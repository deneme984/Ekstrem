 ...
 * @returns {string} Formatted date e.g. "24 May─▒s 2026", or "" on invalid input
 *
 * @example
 * formatTurkishDate("2026-05-24");  // "24 May─▒s 2026"
 ...
 * @returns {string} Turkish month name e.g. "May─▒s", or "" on invalid input
 *
 * @example
 * getMonthName(5);   // "May─▒s"
 * getMonthName(12);  // "Aral─▒k"
 * getMonthName(0);   // ""
 */
...
/**
 * Returns a human-readable relative label for a date (Bug├╝n, Yar─▒n, X g├╝n sonra, X g├╝n ge├ºti).
 *
 * @param {string|null|undefined} isoDate - ISO date string
 * @returns {string} Human-readable label in Turkish
 *
 * @example
 * getRelativeLabel("2026-06-03");  // "Bug├╝n"
 * getRelativeLabel("2026-06-04");  // "Yar─▒n"
 * getRelativeLabel("2026-06-10");  // "7 g├╝n sonra"
 * getRelativeLabel("2026-05-30");  // "4 g├╝n ge├ºti"
 */
export function getRelativeLabel(isoDate) {
  const days = getDaysUntil(isoDate);
  if (days === 0) return 'Bug├╝n';
  if (days === 1) return 'Yar─▒n';
  if (days > 1) return `${days} g├╝n sonra`;
  return `${Math.abs(days)} g├╝n ge├ºti`;
}
