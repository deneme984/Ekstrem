/**
 * Denizbank — PDF Statement Parser
 * 
 * Card products: Bonus, Bonus Trink, Bonus Business, Çözüm Kart
 * PDF: PASSWORD PROTECTED with TCKN (11 digits)
 * Transaction table: İşlem Tarihi | Açıklama | Tutar | Bonus
 * Installments: "X. Taksit / Y Taksit" in description
 * Statement header: "KREDİ KARTI HESAP ÖZETİ"
 */

import BaseParser from '../baseParser.js';

class DenizbankParser extends BaseParser {
  constructor(pdfBuffer) {
    super(pdfBuffer, 'denizbank');
  }

  async parse() {
    await this.extractText();

    // Denizbank PDFs are TCKN-encrypted
  ...
  _extractCardProduct() {
    ...
    if (this.rawText.includes('Çözüm Kart')) return 'Çözüm Kart';
    if (this.rawText.includes('Asya Pasifik')) return 'DenizBank Asya Pasifik';
    return 'Bonus';
  }
  ...
  parseTransactions() {
    ...
    // Denizbank format: DD.MM.YYYY  DESCRIPTION  AMOUNT  [BONUS_POINTS]
    // Installment: "X. Taksit / Y Taksit" or "X/Y Taksit"
  ...
  _isSummaryRow(desc) {
    const keywords = ['dönem borcu', 'asgari', 'toplam', 'limit', 'faiz', 'bonus özet', 'önceki'];
    const lower = desc.toLowerCase();
    return keywords.some(kw => lower.includes(kw));
  }
}

export default DenizbankParser;
