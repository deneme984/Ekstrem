/**
 * Şekerbank — PDF Statement Parser
 * 
 * Card products: Şeker Bonus, Şeker Bonus Gold, Şeker Bonus Platinum, Şeker Kart
 * PDF: NOT password protected
 * Transaction table: Tarih | Açıklama | Tutar | Bonus
 * Installments: "X/Y" in açıklama field
 * Statement header: "KREDİ KARTI HESAP ÖZETİ — ŞEKERBANK"
 * Note: Mid-size bank, simpler layout than tier-1 banks
 */

import BaseParser from '../baseParser.js';

class SekerbankParser extends BaseParser {
  constructor(pdfBuffer) {
    super(pdfBuffer, 'sekerbank');
  }
  ...
  _extractCardProduct() {
    if (this.rawText.includes('Şeker Bonus Platinum')) return 'Şeker Bonus Platinum';
    if (this.rawText.includes('Şeker Bonus Gold')) return 'Şeker Bonus Gold';
    if (this.rawText.includes('Şeker Kart')) return 'Şeker Kart';
    if (this.rawText.includes('Şeker Bonus') || this.rawText.includes('Seker Bonus')) return 'Şeker Bonus';
    return 'Şekerbank Kredi Kartı';
  }
  ...
  parseTransactions() {
    const transactions = [];
    // Şekerbank: simpler layout, Tarih | Açıklama | Tutar | Bonus
    ...
    while ((match = txPattern.exec(workText)) !== null) {
      ...
      const description = this.cleanMerchantName(
        rawDesc.replace(/\d+\s*\/\s*\d+/g, '').trim()
      );
  ...
  _isSummaryRow(desc) {
    const keywords = ['dönem borcu', 'asgari', 'toplam', 'limit', 'faiz', 'bonus', 'önceki'];
    const lower = desc.toLowerCase();
    return keywords.some(kw => lower.includes(kw));
  }
}

export default SekerbankParser;
