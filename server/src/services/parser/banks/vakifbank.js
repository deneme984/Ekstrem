/**
 * Vakıfbank — PDF Statement Parser
 * 
 * Card products: World, World Gold, World Platinum, Chip-Para, Vakıf Card, Miles & Smiles
 * PDF: PASSWORD PROTECTED with TCKN (11 digits)
 * Transaction table: Tarih | İşlem Adı | Tutar
...
class VakifbankParser extends BaseParser {
  constructor(pdfBuffer) {
    super(pdfBuffer, 'vakifbank');
  }

  async parse() {
    await this.extractText();

    // Vakıfbank PDFs are TCKN-encrypted
  ...
  _extractCardProduct() {
    ...
    if (this.rawText.includes('World Platinum')) return 'World Platinum';
    if (this.rawText.includes('World Gold')) return 'World Gold';
    if (this.rawText.includes('Chip-Para') || this.rawText.includes('Chip Para')) return 'Chip-Para';
    if (this.rawText.includes('Vakıf Card')) return 'Vakıf Card';
    return 'World';
  }
  ...
  parseTransactions() {
    const transactions = [];
    const sectionStart = this.rawText.search(/[İI][ŞS]LEM\s+AD[I]|TAR[İI]H\s+A[ÇC][İI]KLAMA/i);
    const workText = sectionStart > 0 ? this.rawText.slice(sectionStart) : this.rawText;

    // Vakıfbank: DD.MM.YYYY  MERCHANT  AMOUNT
    // Installment "X/Y" in the açıklama field
  ...
  _isSummaryRow(desc) {
    const keywords = ['dönem borcu', 'asgari', 'toplam', 'limit', 'faiz', 'worldpoints', 'önceki'];
    const lower = desc.toLowerCase();
    return keywords.some(kw => lower.includes(kw));
  }
}

export default VakifbankParser;
