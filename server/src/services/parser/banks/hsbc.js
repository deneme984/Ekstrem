/**
 * HSBC Türkiye — PDF Statement Parser
 * 
 * Card products: HSBC Advantage, HSBC Advance, HSBC Premier, HSBC Classic, HSBC Business
 * PDF: NOT password protected
 * Transaction table: Tarih | Açıklama | Taksit | Tutar
...
class HSBCParser extends BaseParser {
  constructor(pdfBuffer) {
    super(pdfBuffer, 'hsbc');
  }
  ...
  _extractCardProduct() {
    if (this.rawText.includes('HSBC Premier')) return 'HSBC Premier';
    if (this.rawText.includes('HSBC Advance')) return 'HSBC Advance';
    if (this.rawText.includes('HSBC Advantage')) return 'HSBC Advantage';
    if (this.rawText.includes('HSBC Business')) return 'HSBC Business';
    if (this.rawText.includes('HSBC Classic')) return 'HSBC Classic';
    return 'HSBC Kredi Kartı';
  }
  ...
  parseTransactions() {
    const transactions = [];
    // HSBC: Tarih | Açıklama | Taksit (X/Y) | Tutar
