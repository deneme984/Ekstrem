/**
 * Ziraat Bankası — PDF Statement Parser
 * 
 * Card products: Bankkart, Bankkart Platinum, Bankkart Gold, Bankkart Classic, Visa Infinite
 * PDF: PASSWORD PROTECTED — last 4 digits of TCKN or full 11-digit TCKN
 * Transaction table: Tarih | İşlem Açıklaması | Tutar | Bakiye
 * Installments: "X/Y Taksit" in açıklama column
 * Statement header: "BANKKART KREDİ KARTI HESAP ÖZETİ"
 */

import BaseParser from '../baseParser.js';

class ZiraatParser extends BaseParser {
  constructor(pdfBuffer) {
    super(pdfBuffer, 'ziraat');
  }

  async parse() {
    await this.extractText();

    // Ziraat PDFs are TCKN-encrypted
  ...
  _extractCardProduct() {
    if (this.rawText.includes('Visa Infinite')) return 'Visa Infinite';
    if (this.rawText.includes('Bankkart Platinum')) return 'Bankkart Platinum';
    if (this.rawText.includes('Bankkart Gold')) return 'Bankkart Gold';
    if (this.rawText.includes('Bankkart Classic')) return 'Bankkart Classic';
    if (this.rawText.includes('Bankkart Combo')) return 'Bankkart Combo';
    return 'Bankkart';
  }
  ...
  parseTransactions() {
    const transactions = [];
    // Ziraat: Tarih | İşlem Açıklaması | Tutar | Bakiye
    // The bakiye column trails after the amount
    const sectionStart = this.rawText.search(/[İI][ŞS]LEM\s+A[ÇC][İI]KLAMA|TAR[İI]H\s+A[ÇC][İI]KLAMA/i);
  ...
  _isSummaryRow(desc) {
    const keywords = ['dönem borcu', 'asgari', 'toplam', 'limit', 'faiz', 'önceki', 'bankkart'];
